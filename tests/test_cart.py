...
class TestCart:
    ...
    @allure.description("Проверка, что в корзине товар отображается корректно: название, количество, цена")
    @pytest.mark.parametrize("browser_name", ["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_cart_items_display(self, subtests, driver, user_authorization, browser_name):
            ...
            for label, actual, expected, custom_assert in data:
                with allure.step(f"Проверка {label}"):
                    with ((subtests.test(label=label))):
                        if custom_assert:
                            custom_assert(actual, expected, f"{label} некорректно отображается в корзине")
                        else:
                            try:
                                assert actual == expected, f"{label}некорректно: {actual} ≠ {expected}"
    ...
    @allure.title("Проверка корректного пересчета итоговой цены в корзине в {browser_name}")
    @allure.description("Проверка, что в корзине итоговая цена пересчитывается корректно при добавлении")
    @pytest.mark.parametrize("browser_name",["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_cart_price_recalculation(self, driver, user_authorization,browser_name):
        allure.dynamic.parameter("Браузер", browser_name)
        with allure.step("Открытие страницы и иницилизация объектов"):
            shop = ShopPage(driver)
            cart = CartPage(driver)
        ...
        with allure.step("Сохранение количества товаров ДО добавления"):
            ...
            allure.attach(f"Количество товара до добавления: {str(quantuty_before_add)}", name="Quantity before add", attachment_type=allure.attachment_type.TEXT)

        with allure.step("Сохранение цены товара"):
            price = float(cart.get_product_1_price().split()[0])
            allure.attach(f"Цена товара: {str(price)}", name="Price", attachment_type=allure.attachment_type.TEXT)

        with allure.step("Сохранение итоговой цены ДО добавления"):
            total_1 = float(cart.get_total().split()[-2])
            allure.attach(f"Итоговая цена ДО добавления: {total_1}", name = "Total before add", attachment_type=allure.attachment_type.TEXT)
        ...
        with allure.step("Сохранение количества товаров ПОСЛЕ добавления"):
            ...
            allure.attach(f"Количество товара до добавления: {str(quantuty_after_add)}", name="Quantity after add", attachment_type=allure.attachment_type.TEXT)

        with allure.step("Сохранение итоговой цены ПОСЛЕ добавления"):
            total_2 = float(cart.get_total().split()[-2])
            allure.attach(f"Итоговая цена ПОСЛЕ добавления {total_2}", name="Total after add", attachment_type=allure.attachment_type.TEXT)

        with allure.step("Проверка корректного пересчета итоговой цены"):
            try:
                assert total_2 == price * quantuty_after_add, "Итоговая цена пересчитывается некорректно"
            except AssertionError as e:
                # В случае несовпадения - логирование ошибки
                allure.attach(str(e), name=f"Текст ошибки", attachment_type=allure.attachment_type.TEXT)
                # Принудительное падение теста
    ...
    @allure.description("Проверка, что корзина корректно очищается и сообщение о пустой корзине корректна")
    @pytest.mark.parametrize("browser_name", ["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_clear_cart(self, driver, user_authorization, browser_name):
        ...
        with allure.step("Сохранить количество товара в корзине ДО удаления"):
            ...
            allure.attach(f"Количество товара ДО удаления {quantity_before_clear}", name="Quantity before clear", attachment_type=allure.attachment_type.TEXT)
        ...
        with allure.step("Проверка корректности сообщения о пустой корзине"):
            ...
            assert_text_equal(empty_cart_message, EMPTY_CART_MESSAGE.lower(),"Корзина не очистилась!")

    @allure.title("Проверка неактивности кнопки 'Оформить заказ' при пустой корзине в {browser_name}")
    @allure.description("Проверка, что в пустой корзине кнопка 'Оформить заказ' не отображается или неактивна")
    @pytest.mark.parametrize("browser_name", ["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_make_order_button_disabled_when_cart_empty(self, driver, user_authorization, browser_name):
        allure.dynamic.parameter("Браузер", browser_name)
        with allure.step("Открытие страницы и иницилизация объектов"):
            shop = ShopPage(driver)
            cart = CartPage(driver)
        ...
        with allure.step("Проверка недоступности кнопки 'Оформить заказ'"):
            ...
            if not is_visible:
                ...
                try:
                    assert not is_enabled, ("Кнопка 'Оформить заказ' активна при пустой корзине")
                except AssertionError as e:
                    # В случае несовпадения - логирование ошибки
                    allure.attach(str(e), name=f"Текст ошибки", attachment_type=allure.attachment_type.TEXT)
                    # Принудительное падение теста
    ...
    @allure.title("Проверка активности кнопки 'Оформить заказ' при непустой корзине в {browser_name}")
    @allure.description("Проверка, что в непустой корзине кнопка 'Оформить заказ' отображается и активна")
    @pytest.mark.parametrize("browser_name", ["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_make_order_button_enabled(self, driver, user_authorization, browser_name):
        allure.dynamic.parameter("Браузер", browser_name)
        with allure.step("Открытие страницы и иницилизация объектов"):
            shop = ShopPage(driver)
            cart = CartPage(driver)
        ...
        with allure.step("Проверка доступности кнопки 'Оформить заказ'"):
            is_visible = cart.is_make_order_button_visible()
            try:
                assert is_visible, ("Кнопка 'Оформить заказ' не видима при непустой корзине")
            except AssertionError as e:
                # В случае несовпадения - логирование ошибки
                allure.attach(str(e), name=f"Текст ошибки", attachment_type=allure.attachment_type.TEXT)
                # Принудительное падение теста
            ...
            is_enabled = cart.is_make_order_button_enabled()
            try:
                assert is_enabled, ("Кнопка 'Оформить заказ' неактивна при непустой корзине")
            except AssertionError as e:
                # В случае несовпадения - логирование ошибки
                allure.attach(str(e), name=f"Текст ошибки", attachment_type=allure.attachment_type.TEXT)
                # Принудительное падение теста
    ...
    @allure.title("Проверка работоспособности кнопки 'Оформить заказ' в {browser_name}")
    @allure.description("Проверка, что в непустой корзине кнопка 'Оформить заказ' переводит на страницу заполнения личных данных")
    @pytest.mark.parametrize("browser_name", ["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_making_order_button_work(self, driver, user_authorization, browser_name):
        allure.dynamic.parameter("Браузер", browser_name)
        with allure.step("Открытие страницы и иницилизация объектов"):
            shop = ShopPage(driver)
            cart = CartPage(driver)
        ...
        with allure.step("Проверка нахождения на странице заполнения личных данных"):
            ...
            assert_text_equal(making_order_text, MAKE_ORDER_HEADER_TEXT.lower(),"Кнопка Оформить заказ не переводит на страницу заполнения данных")