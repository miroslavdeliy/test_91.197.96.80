...
class TestPerformance:
    ...
    @allure.title(
        "Проверка времени загрузки главной страницы в  {browser_name}"
    )
    @allure.description(
        "Проверка что время загрузки главной страницы не превышает допустимое"
    )
    ...
    def test_time_load(self, driver, browser_name):
        ...
        with allure.step(
                "Получение времени начала и окончания загрузки страницы"
        ):
            ...
            allure.attach(
                str(load_time_ms),
                name=f"Время загрузки",
                attachment_type=allure.attachment_type.TEXT
            )

        with allure.step(
                "Проверка, что время загрузки не превышает допустимое"
        ):
            try:
                assert load_time_ms <= MAX_LOAD_TIME_MS,\
                    (f"Главная страница загружалась слишком долго: "
                     f"{load_time_ms} мс")
