...
class TestProductCart:
    ...
    @allure.description("Проверка, что в карточке товара присутствует название, описание, цена, изображение")
    @pytest.mark.parametrize("browser_name", ["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_product_card_display(self, subtests, driver, user_authorization, browser_name):
        ...
        with allure.step("Проверка загрузки изображения"):
            ...
            try:
                assert image.get_attribute("src") != "", "Изображение товара не загружено (src-пустой)"
    ...
    @allure.description("Проверка, что кнопки изменения количества товара увеличивают и уменьшают количество товара")
    @pytest.mark.parametrize("browser_name", ["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_button_changing_quantity_goods(self, subtests, driver, user_authorization, browser_name):
        allure.dynamic.parameter("Браузер", browser_name)
        with allure.step("Открытие страницы и иницилизация объектов"):
            shop = ShopPage(driver)
        ...
        with allure.step("Получить текущее количество (до добавления)"):
            ...
            allure.attach(f"Количество товаров до добавления: {quantity_before_add}", name="Quantity before add", attachment_type=allure.attachment_type.TEXT)
        ...
        with allure.step("Получить текущее количество (после добавления)"):
            ...
            allure.attach(f"Количество товаров после добавления: {quantity_after_add}", name="Quantity after add", attachment_type=allure.attachment_type.TEXT)
        ...
        with allure.step("Проверка увеличения количества товара на 1 штуку"):
            with subtests.test(label="Добавление товара"):
                try:
                    assert quantity_after_add == quantity_before_add + 1, "Количество товара не увеличилось!"
        ...
        with allure.step("Получить текущее количество (после удаления)"):
            ...
            allure.attach(f"Количество товаров после удаления: {quantity_after_remove}", name="Quantity after remove", attachment_type=allure.attachment_type.TEXT)
        ...
        with allure.step("Проверка уменьшения количества товара на 1 штуку"):
            with subtests.test(label="Удаление товара"):
                try:
                    assert quantity_after_remove == quantity_after_add - 1, f"Количество товара не уменьшилось!"
    ...
    @allure.title("Проверка видимости изменения количества товара в корзине {browser_name}")
    @allure.description("Проверка, что кнопки изменения количества товара увеличивают и уменьшают количество товара в корзине")
    @pytest.mark.parametrize("browser_name",["Mozilla Firefox", "Google Chrome", "Microsoft Edge", "Yandex Browser"])
    def test_visibility_changing_quantity_goods_in_cart(self, subtests, driver, user_authorization, browser_name):
        allure.dynamic.parameter("Браузер", browser_name)
        with allure.step("Открытие страницы и иницилизация объектов"):
            shop = ShopPage(driver)

        with allure.step("Получить текущее количество товаров в корзине (до добавления)"):
            quantity_before_add = int(shop.get_quantity_goods_in_cart())
            allure.attach(f"Количество товаров в корзине до добавления: {quantity_before_add}", name="Quantity before add", attachment_type=allure.attachment_type.TEXT)
        ...
        with allure.step("Получить текущее количество в корзине (после добавления)"):
            ...
            allure.attach(f"Количество товаров в корзине после добавления: {quantity_after_add}", name="Quantity after add", attachment_type=allure.attachment_type.TEXT)

        # Проверка увеличения количества товара на 1 штуку
        with allure.step("Проверка увеличения количества товара в корзине на 1 штуку"):
            with subtests.test(label="Добавление товара"):
                try:
                    assert quantity_after_add == quantity_before_add + 1, f"Количество товара не увеличилось Было: "
                except AssertionError as e:
                    allure.attach(str(e), name=f"Текст ошибки", attachment_type=allure.attachment_type.TEXT)
                    assert False, str(e)
        ...
        # Получаем количество товаров после удаления
        with allure.step("Получить текущее количество товаров в корзине (после удаления)"):
            quantity_after_remove = int(shop.get_quantity_goods_in_cart())
            allure.attach(f"Количество товаров в корзине после удаления: {quantity_after_remove}", name="Quantity after remove", attachment_type=allure.attachment_type.TEXT)

        # Проверка уменьшения количества товара на 1 штуку
        with allure.step("Проверка уменьшения количества товара в корзине на 1 штуку"):
            with subtests.test(label="Удаление товара"):
                try:
                    assert quantity_after_remove == quantity_after_add - 1, f"Количество товара не уменьшилось!"
