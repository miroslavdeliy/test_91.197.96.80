...
class TestNegative:
    @allure.title(
        "Проверка добавления товаров в корзину больше максимального в "
        "{browser_name}"
    )
    @allure.description(
        "Проверка добавления в корзину товаров больше максимального и что "
        "система не позволяет добавить больше"
    )
    ...
    def test_more_max_goods(self, driver, user_authorization, browser_name):
        ...
        with allure.step(
                "Добавление товаров в корзину больше максимального на 1"
        ):
            for _ in range(MAX_QUANTITY + 1):
                shop.add_product_1()

        with allure.step(f"Проверка, что в корзине не больше {MAX_QUANTITY}"
                         f" штук"):
            count_goods = shop.get_quantity_goods_in_cart()
            try:
                assert int(count_goods) <= MAX_QUANTITY, \
                    f"В корзину добавляется больше {MAX_QUANTITY} единиц"
    ...
    @allure.title(
        "Проверка добавления товаров в корзину на стоимость больше "
        "максимальной возможной в {browser_name}"
    )
    @allure.description(
        "Проверка добавления в корзину товаров больше максимальной стоимость "
        "и что система не позволяет добавить больше"
    )
    @pytest.mark.parametrize(
        "browser_name",
        [
    ...
    def test_more_max_total(self, driver, user_authorization, browser_name):
        ...
        with allure.step(
                f"Увеличивать, пока общая сумма не превысит {MAX_TOTAL}"
        ):
            current_total = 0
        ...
        with allure.step(
                f"Проверка, что обшая сумма не превышает {MAX_TOTAL}"
        ):
            try:
    ...
    @allure.title(
        "Проверка оформления заказа без заполнения личных данных в "
        "{browser_name}"
    )
    @allure.description(
        "Проверка оформления заказа без заполнения личных данных и "
    ...
    def test_without_personal_data(
        ...
        with allure.step(
                "Нажать на кнопку Оформить заказ без заполнения личных данных"
        ):
